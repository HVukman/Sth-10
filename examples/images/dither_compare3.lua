-- compare ditherings (black and white)
...
local function init()
    ...
    window.title("Compare Ditherings 2")
    window.set_window_position(200,100)
    window.set_width_height(width,height)

    Img_ = image.load_image("examples/resources/David.png")
    ...
    Img_ = image.dither_atkinson(Img_,1)
    Img2_ = image.dither_jarvis(Img2_,1)
...
local function draw()
    ...
    local textsize = 22
    text_.draw_text("Atkinson Dither", P3, textsize, color.GREEN)
    text_.draw_text("Jarvis Dither", P4, textsize, color.GREEN)
end
