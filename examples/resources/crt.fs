// https://www.shadertoy.com/view/ldXSz4 -- dithering?
...
float hash(vec2 p) { return fract(1e4 * 
sin(17.0 * p.x + p.y * 0.1) * (0.1 + abs(sin(p.y * 13.0 + p.x)))); }
...
#	undef TRY	
	...
	finalColor = vec4(mod(c.x + c.y, 2.0) >  (hash(c * 2.0 + fract(sin(vec2(floor(time), floor(time * 1.7))))) * 0.75) + (best1 / (best0 + best1)) ? dst1 : dst0, 1.0);
}