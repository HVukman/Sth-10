...
lua_draw_catmull_rom_spline :: proc "c" (L: ^lua.State) -> i32 {
  ...
  lua_draw_basis_spline :: proc "c" (L: ^lua.State) -> i32 {

   // const Vector2 *points, int pointCount, float thick, Color color
   // draw.basis_spline(x,y,thick, color)
     	context = runtime.default_context()
     	a := cast(^array.array)lua.touserdata(L, 1)
      b := cast(^array.array)lua.touserdata(L, 2)
      thick:= lua.L_checknumber(L,3)
      ...
      if a.size <4 || b.size <4 {
      	...
      	lua.L_error(L,"not enough points for spline (at least 4)")
      	return 0
      } else if a.size != b.size {
	   	lua.L_error(L,"arrays must be same size")
	   	return 0
	...
	dummy := raw_data(points)
      rl.DrawSplineBasis(dummy,i32(a.size),f32(thick) , COLOR_ARRAY[int(col_)])
   ...
   lua_draw_bezier_cubic_spline :: proc "c" (L: ^lua.State) -> i32 {

    // const Vector2 *points, int pointCount, float thick, Color color
    // draw.bezier_cubic_spline(x,y,thick, color)
      	context = runtime.default_context()
      	a := cast(^array.array)lua.touserdata(L, 1)
       b := cast(^array.array)lua.touserdata(L, 2)
       thick:= lua.L_checknumber(L,3)
       ...
       if a.size <4 || b.size <4 {
       	...
       	lua.L_error(L,"not enough points for spline (at least 4 ; 2 control points)")
       	return 0
       } else if a.size != b.size {
	   	lua.L_error(L,"arrays must be same size")
	   	return 0
	...
	dummy := raw_data(points)
       rl.DrawSplineBezierCubic(dummy,i32(a.size),f32(thick) , COLOR_ARRAY[int(col_)])
    ...
    lua_draw_bezier_quadratic_spline :: proc "c" (L: ^lua.State) -> i32 {

     // const Vector2 *points, int pointCount, float thick, Color color
     // draw.bezier_quadratic_spline(x,y,thick, color)
       	context = runtime.default_context()
       	a := cast(^array.array)lua.touserdata(L, 1)
        b := cast(^array.array)lua.touserdata(L, 2)
        thick:= lua.L_checknumber(L,3)
        ...
        if a.size <3 || b.size <3 {
        	...
        	lua.L_error(L,"not enough points for spline (at least 3 ; 1 control point)")
        	return 0
        } else if a.size != b.size {
	   	lua.L_error(L,"arrays must be same size")
	   	return 0
	...
	dummy := raw_data(points)
        rl.DrawSplineBezierQuadratic(dummy,i32(a.size),f32(thick) , COLOR_ARRAY[int(col_)])
